...
export class LeftSidebarComponent implements OnInit {
  ...
  subscribeToUserConfigurationAndArticles() {
    this.subscription.push(this._configurationService.openConfiguration$.subscribe(
     notification => {
          let number = this._configurationService.getParam('number_of_articles_in_sidebar');
    ...
    this.subscription.push(
      this._articleService.notify$.subscribe(
        param => {
  ...
  filterTexts(){
    for (let tag of this.tags){
      for (let article of this.allArticles){
        ...
        if (articleTags.indexOf(tag.name) !== -1 && !this.isArticleAssinged(article)) {
          tag.texts.push(article);
        }
      }
      //console.log('Before sorting:',tag.texts);
      tag.texts.sort( function(article1,article2){
        let diff = article1.tags.indexOf(tag.name) - article2.tags.indexOf(tag.name);
        return diff;
      });
